...
void continue_execution() {
    if(ptrace(PTRACE_CONT, child_pid, nullptr, nullptr) < 0) errquit("ptrace(CONT)");
    ...
    if(WIFSTOPPED(status)) {
        struct user_regs_struct regs;
        if(ptrace(PTRACE_GETREGS, child_pid, nullptr, &regs) < 0) errquit("ptrace(GETREGS)");
        printf("** program stopped at 0x%llx\n", regs.rip);
        disassemble(child_pid, regs.rip);
    }
}
